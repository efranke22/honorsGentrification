...
Fix some common mistakes. This gets us about 6,600 more of the missing crimes. 
...
```{r}
sp_crimes_located <- bind_rows(address_clean, intersection_crimes)
```
...
Aggregate the points by census tract. 
```{r}
tract_points_sp <- st_join(sp_sf, left = FALSE, gent_clean["tract"]) 
...
tract_aggregated_sp <- st_join(gent_clean, crimes_per_tract_sp, left = FALSE) %>%
  ...
  mutate(crimes_per_1000 = (n/population_2010) * 1000, 
         crimes_per_1000d = as.factor(case_when(crimes_per_1000 <=23 ~ "0-23", 
                                      ...
                                      TRUE ~ "75+")), 
         crimes_per_1000d = fct_relevel(crimes_per_1000d, levels = c("0-23", "24-36", "37-52", "53-75", "75+"))) %>%
  filter(county == "Ramsey", !(tract %in% c(409.01, 408.01, 405.02, 402, 411.07, 408.03, 408.02, 412, 404.01, 404.02, 413.02, 414, 415, 413.01, 416.01, 424.01, 426.01, 427, 424.02, 425.01)))
```
...
```{r,eval=FALSE}
save(tract_aggregated_sp, file = 'Data/tract_aggregated_sp.RData')
save(tract_points_sp, file = "Data/tract_points_sp.RData")
```
