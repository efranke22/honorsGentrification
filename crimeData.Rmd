...
```{r}
total_crimes_per_tract <- tract_points %>%
  group_by(tract, year) %>%
  count() %>%
  rename("total" = "n")
...
```{r}
sp_violent <- tract_points_sp %>%
  filter(incident_group == "Assault") %>%
  group_by(tractmod, year) %>%
  count()

tract_aggregated_violent_sp <- st_join(gent_clean, sp_violent, left = FALSE) %>%
  select(-tractmod.y) %>%
  rename("tractmod" = "tractmod.x") %>%
  mutate(crimes_per_1000 = (n/population_2020) * 1000)
```
...
```{r}
theft_sp<- tract_points_sp %>%
  filter(incident_group == "Theft") %>%
  group_by(tractmod, year) %>%
  count()

tract_aggregated_theft_sp <- st_join(gent_clean, theft_sp, left = FALSE) %>%
  select(-tractmod.y) %>%
  rename("tractmod" = "tractmod.x") %>%
  mutate(crimes_per_1000 = (n/population_2020) * 1000)
```
...
```{r}
sp_autotheft <- tract_points_sp %>%
  filter(incident_group == "Auto Theft") %>%
  group_by(tractmod, year) %>%
  count()

tract_aggregated_autotheft_sp <- st_join(gent_clean, sp_autotheft, left = FALSE) %>%
  select(-tractmod.y) %>%
  rename("tractmod" = "tractmod.x") %>%
  mutate(crimes_per_1000 = (n/population_2020) * 1000)
```
...
save(tract_aggregated_autotheft_sp, file = "Data/CrimeCleaned/tract_aggregated_sp_autotheft.RData")
```