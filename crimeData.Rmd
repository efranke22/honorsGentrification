...
minneapolisCrime %>%
  count(Offense, year) %>%
  filter(Offense %in% c("THEFT", "TFMV", "BURGD", "AUTOTH")) %>%
  ggplot(aes(x=as.numeric(year), y=n, color = Offense))+
  geom_point()+
  geom_line()+
  theme_classic()+
...
sp_sf <- sp_crimes_located %>%
  ...
  mutate(OffenseCat = case_when(INCIDENT %in% c("Theft", "Burglary", "Vandalism", "Auto Theft", "Narcotics", "Criminal Damage", "Graffiti", "THEFT") ~ "Theft", 
...
crimes_per_tract_no_geo_sp <- tract_points_sp %>%
  st_drop_geometry() %>%
  group_by(tractmod, year, OffenseCat, .drop=FALSE) %>%
  count() %>%
  pivot_wider(names_from = year, values_from = n) %>%
...
tract_aggregated_sp <- gent_clean %>%
  right_join(crimes_per_tract_no_geo_sp, by = "tractmod") %>%
   filter(county_2020 == "Ramsey") %>%
  mutate(violent_per_1000 = case_when(year %in% c(2014, 2015) ~ round((Violent/population_2010) * 1000,0),
                                      TRUE ~ round((Violent/population_2020) * 1000,0)), 
         theft_per_1000 = case_when(year %in% c(2014, 2015) ~ round((Theft/population_2010) * 1000,0),
                                      TRUE ~ round((Theft/population_2020) * 1000,0)))
```
