...
Create a final gentrified data set. First, deal with the weird 2010 tracts by creating `gentrifiable_groups`.
```{r}
gentrifiable_groups <- gentrifiable %>%
  st_drop_geometry() %>%
  filter(tract %in% c(249.01, 249.02, 1023.00, 1029.00, 408.02, 408.03)) %>%
  mutate(grouped = case_when(tract %in% c(249.01, 249.02) ~ 249.04, 
                             tract %in% c(1023.00, 1029.00) ~ 1263.00,
                             TRUE ~ 408.04), 
         canGent = as.numeric(canGent)-1) %>%
  group_by(grouped) %>%
  summarize(canGent = as.factor(mean(canGent))) %>%
  rename("tract" = "grouped") %>%
  mutate(county = c("Hennepin", "Ramsey", "Hennepin"))
```

Create final `gentrifiable_clean` dataset, which is the same as the original `gentrifiable` but just includes the manipulated tracts.
```{r}
gentrifiable_clean <- gentrifiable %>%
  filter(!(tract %in% c(249.01, 249.02, 1023.00, 1029.00, 408.02, 408.03))) %>%
  bind_rows(gentrifiable_groups) %>%
  st_drop_geometry() %>%
  rename("tractmod" = "tract")
```  

Final dataset which lists whether each or not each census tract was gentrifiable and if it did gentrify according to Ding and Freeman's methods.
```{r}
gentrified <- final %>%
  group_by(tractmod) %>%
  ...
  left_join(gentrifiable_clean %>% select(tractmod, canGent, county), by = "tractmod") %>%
...
Understand what proportion of tracts that could gentrify did. Out of the 226 tracts, Freeman and Ding agreed that 129 did not gentrify and 44 did. Freeman's methods yielded 45 gentrifying that Ding did not, while Ding's methods identified 8 gentrifying the Freeman did not.
```{r}
gentrified %>%
  filter(canGent == "1") %>%
  ungroup()%>%
