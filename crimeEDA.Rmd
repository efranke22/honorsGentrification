...
In this file I create exploratory graphs using cleaned Minneapolis and Saint Paul crime data. 
...
tract_aggregated %>%
  ...
  theme(axis.line = element_blank(), 
        ...
        axis.ticks = element_blank(),
        plot.title = element_text(hjust=0.5))+
  labs(title = "2020 Minneapolis violent crime count", fill = "Count")
...
mn2020 <- tract_aggregated %>%
  filter(year == 2020) %>%
  dplyr::select(tractmod, year, Violent) %>%
  mutate(violentD= case_when(Violent <=10 ~ "0-10",
                               Violent >10 & Violent  <=25 ~ "11-25",
                               Violent >25 & Violent <= 50 ~ "26-50", 
                               Violent >50 & Violent <= 100 ~ "51-100",
...
bind_rows(mn2020, sp2020) %>%
  ggplot()+
  geom_sf(aes(fill =violentD), override.order=TRUE)+
  scale_fill_brewer(palette = "Blues")+
  labs(title = "2020 violent crime by census tract in Minneapolis-Saint Paul", fill = "", y = "", x = "")+
   theme(axis.line = element_blank(), 
        axis.text = element_blank(), 
        axis.ticks = element_blank())+
  annotate("point", x = -93.1691, y = 44.938, colour = "black", size = 1) +
  annotate(geom = "text", x = -93.1691, y = 44.939, label = "Macalester \nCollege", size = 2, color = "black", family = "Coming Soon")
```
