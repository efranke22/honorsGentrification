...
mn2020 <- tract_aggregated %>%
  filter(year == 2020) %>%
  select(tractmod, year, violent_per_1000) %>%
  mutate(violent_d = case_when(violent_per_1000 <=4 ~ "0-4",
                               violent_per_1000 >4 & violent_per_1000 <=9 ~ "5-9",
                               violent_per_1000 >9 & violent_per_1000 <= 16 ~ "10-16", 
                               TRUE ~ "17+"))
...
bind_rows(mn2019, sp2019) %>%
  mutate(violent_d = fct_relevel(violent_d, levels = c("0-4", "5-9", "10-16", "17+"))) %>%
  ggplot()+
  geom_sf(aes(fill =violent_d))+
  theme_classic()+
